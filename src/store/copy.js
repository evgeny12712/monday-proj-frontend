...
//       const idx = state.boards.findIndex(({ _id }) => _id === board._id);
//       if (idx === -1) state.boards.push(board);
//       else state.boards.splice(idx, 1, board);
//     },
//     removeBoard(state, { boardId }) {
//       const idx = state.boards.findIndex(({ _id }) => _id === boardId);
...
//       const toBoard = state.boards.find(({ _id }) => _id === board._id);
...
//       else if (idx === -1 && task.title === 'New task')
...
//     },
//     saveCol(state, { col }) {
//       const idx = state.activeBoard.cols.findIndex(
//         ({ type }) => type === col.type
//       );
//       if (idx === -1) state.activeBoard.cols.push(col);
//       else state.activeBoard.cols.splice(idx, 1, col);
//     },
//   },
//   actions: {
//     //----------BOARD----------//
...
//         context.dispatch({
//           type: 'addMention',
//           mention: { boardId: context.state.activeBoard, group: addedGroup },
//         });
//         return addedGroup;
//       } catch (err) {
//         return err;
...
//         const activeBoard = context.state.activeBoard;
//         context.commit({ type: 'saveTask', ...detailsToSend });
//         const newBoard = await boardService.saveTask(
//           detailsToSend,
//           activeBoard
//         );
//         console.log('DONE', context.state.activeBoard.groups[0]);
//         return newBoard;
//       } catch (err) {
//         return err;
//       }
...
//       try {
//         const detailsToSend = {
//           board: context.state.activeBoard,
//           ...details,
//         };
//         context.commit({ type: 'deleteTask', ...detailsToSend });
//         const newBoard = await boardService.deleteTask(detailsToSend);
//         return newBoard;
//       } catch (err) {
//         return err;
//       }
...
//       try {
//         const savedCol = await boardService.saveLabel(
//           context.state.activeBoard._id,
//           details
//         );
//         context.commit({ type: 'saveCol', col: savedCol });
//         return savedCol.data.opts;
//       } catch (err) {
//         return err;
//       }
//     },
