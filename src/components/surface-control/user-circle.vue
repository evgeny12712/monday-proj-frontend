<template>
  <section class="surface-control-users" :style="{ backgroundColor: colorToShow }" @click="toggleUserMenu">
    {{ textToShow }}
    <div v-if="isUserMenuOpen" class="surface-control-user-menu" v-click-outside="toggleUserMenu">
      <div v-if="isFromSurface" class="surface-control-user-menu-item" @click="logOut">
        <div v-html="signIcon" class="menu-icon-wrapper"></div>
        {{ signBtn }}
      </div>
      <div class="surface-control-user-menu-item" @click="openProfile">
        <div class="menu-icon-wrapper">
          <svg viewBox="0 0 20 20" fill="currentColor" width="16" height="16" aria-hidden="true" aria-label="My profile" class="icon_component icon_component--no-focus-style">
            <path
...
export default {
  ...
  methods: {
    toggleUserMenu() {
      if (this.isFromSurface) this.isUserMenuOpen = !this.isUserMenuOpen;
    },
    async logOut() {
      this.$emit('logout');
    },
  ...
  computed: {
    ...
    colorToShow() {
      return this.user ? this.user.color : 'grey';
    },
    signIcon() {
    ...
    textToShow() {
      return this.user ? this.user.username.charAt(0) : 'G';
    },
  },
};
</script>
